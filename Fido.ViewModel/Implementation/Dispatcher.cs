...
namespace Fido.ViewModel.Implementation
{
    public class Dispatcher<TRETURN> : IDispatcher<TRETURN> where TRETURN : class
    {
            ...
            where TMODEL : IModel<TMODEL>
        {
            ...
            var Redirect = CheckPermissions(Model, ReadWrite.Read);
        ...
        public TRETURN Load<TMODEL>(Func<TMODEL, TRETURN> Result)
            where TMODEL : IModel<TMODEL>
        {
            return View<TMODEL>(Result, ReadWrite.Write);
        }
            ...
            where TMODEL : IModel<TMODEL>
        {
            using (new FunctionLogger(Log))
            {
                ...
                var Redirect = CheckPermissions(Model, ReadWrite.Write);
        ...
        private TRETURN View<TMODEL>(Func<TMODEL, TRETURN> Result, ReadWrite Function)
            where TMODEL : IModel<TMODEL>
        {
            using (new FunctionLogger(Log))
            {
        ...
        private IList<string> GetDenied()
        {
            ...
            if (AuthenticationAPI.Authenticated)
            {
                ...
                {
                    List.Add(Identifier(Activity.ReadWrite, Activity.Name, Activity.Area));
                }
            }
        ...
        private TRETURN CheckPermissions<TMODEL>(TMODEL DataModel, ReadWrite Function)
            where TMODEL : IModel<TMODEL>
        {
            using (new FunctionLogger(Log))
            {
                ...
                if (Function == ReadWrite.Read && DataModel.ReadAccess != Access.NA ||
                    Function == ReadWrite.Write && DataModel.WriteAccess != Access.NA)
                {
                    if (!AuthenticationAPI.Authenticated)
